...
try {
  ...
  console.log("SignUpScreen: Could not load header logo icon. Check path:", error.message);
}
...
const SignUpScreen = ({ navigation }) => {
  ...
  // Destructure the signUp function and other necessary states from AuthContext
  ...
  const handleSignUp = async () => {
    ...
    const result = await signUp(email.trim(), password); // Use trimmed email
    ...
    if (result && result.success) {
      ...
      setConfirmPassword('');
        ...
        // setError is already called in AuthContext, but we ensure it's handled
        // The error state from context will update the UI
        console.log("Sign up failed on screen:", result.error);
    }
    // If result is undefined or doesn't have success, AuthContext's setError would have been called
  };

  useEffect(() => {
    const unsubscribeFocus = navigation.addListener('focus', () => {
        setError(null); // Clear error on screen focus
        // Clear form fields when screen comes into focus for a fresh start
        // setEmail('');
        // setPassword('');
        // setConfirmPassword('');
    });
  ...
  useEffect(() => {
    let timer;
    if (error) {
      timer = setTimeout(() => setError(null), 3500);
    }
  ...
  const handleFooterPress = () => {
    Alert.alert(
      ...
      [{ text: "I agree" }] // Changed from "OK" to "I agree" as per your code
    );
  };
        ...
        <View style={styles.headerCenterContainer}>
          {headerLogoIcon && (
            <Image
              ...
              onError={(e) => console.log("SignUpScreen: Error loading header logo:", e.nativeEvent.error)}
            />
          )}
        ...
        >
          <View style={styles.card}>
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                style={styles.input}
                placeholder="Choose a strong password (min. 6)"
                placeholderTextColor={Colors.placeholder || '#A0A0A0'}
                value={password}
            ...
            <TouchableOpacity
              ...
              disabled={isLoading}
            >
              {isLoading ? <ActivityIndicator color={Colors.white} /> : <Text style={styles.actionButtonText}>Sign Up</Text>}
            </TouchableOpacity>
            <TouchableOpacity style={styles.signInLinkContainer} onPress={() => { setError(null); navigation.navigate('Login');}}>
              <Text style={styles.signInText}>
                Already have an account?{' '}
                <Text style={styles.signInActionText}>Login</Text>
              </Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  actionButton: { backgroundColor: Colors.primary, paddingVertical: 15, borderRadius: 8, alignItems: 'center', justifyContent: 'center', marginTop: 15, marginBottom: 25 },
  buttonDisabled: { backgroundColor: Colors.grey },
  actionButtonText: { color: Colors.white, fontSize: 16, fontWeight: 'bold' },
  signInLinkContainer: { alignItems: 'center', marginTop: 10 },
  signInText: { fontSize: 14, color: Colors.textDark || '#555555' },
  signInActionText: { color: Colors.secondary || '#3498DB', fontWeight: 'bold' },
